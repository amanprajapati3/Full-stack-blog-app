...
const Sports = () => {
  ...
  const Sports_blog = blogs.filter((blog) => blog.category === "sports");

  return (
    <>
      <h1 className="font-bold font-sans text-3xl sm:text-5xl  text-center pb-5">Sports</h1>
      <div className="mt-5 md:mb-12 mb-4 mx-5 flex justify-center">
        <div className="flex flex-wrap gap-5">
          <div className="flex justify-center md:gap-10 gap-4 flex-wrap lg:mx-20 md:mx-5 sm:mx-2 mx-2 ">
            {loading ? (
              ...
              Sports_blog.slice(0, 4).map((blog) => {
              return (
                 <div
                  key={blog._id}
                ...
                >
                  ...
                  <h1 className="pl-1 pt-3 text-center text-xl font-bold ">{blog.title}</h1>
       ...
       <center>
        <NavLink to={'/sports-blog'}>
        <button className="mb-10 cursor-pointer px-5 py-2 rounded-md opacity-95 hover:opacity-100 transition-all active:scale-95 duration-300 bg-fuchsia-600 hover:bg-fuchsia-800">Load More...</button>
        </NavLink>
      </center>
    </>
  );
